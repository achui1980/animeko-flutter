...
/// endpoint groups (trends, home recommendations, search, schedule,
/// my-collections) return a shared wire model -- each has its own
/// incompatible field set (different names for id/image, inconsistent
/// presence of nameCn/score/tags). Rather than have the UI branch on
/// four different types, every API-specific model is mapped into this
/// one type via a factory constructor added in the task that introduces
/// that API (see SubjectCard.fromTrending, .fromRecommendation,
/// .fromSearchResult, .fromScheduledSubject, .fromMyCollectionSubject).
class SubjectCard {
  const SubjectCard({
    required this.id,
    required this.name,
