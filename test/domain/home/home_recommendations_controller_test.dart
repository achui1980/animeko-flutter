...
void main() {
  ...
  group('build', () {
    test(
      'fetches the first page at offset 0 and maps to SubjectCard',
      () async {
        ...
        final result = await container.read(
          homeRecommendationsControllerProvider.future,
        );
    ...
    test('surfaces an error from the API', () async {
      when(
        () => api.getRecommendations(offset: 0, limit: 20),
  ...
  group('loadMore', () {
    test(
      ...
      () async {
        ...
        await container
            .read(homeRecommendationsControllerProvider.notifier)
            .loadMore();

        final result = container
            .read(homeRecommendationsControllerProvider)
            .value!;
