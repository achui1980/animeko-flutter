...
void main() {
  ...
  group('build', () {
    ...
    test('fetches all types (type: null) at offset 0', () async {
      when(
        () => api.getMyCollections(type: null, offset: 0, limit: 20),
      ).thenAnswer(
        (_) async => const PaginatedCollections(items: [], total: 0),
      );

      final result = await container.read(
        myCollectionsControllerProvider(type: null).future,
      );
    ...
    test(
      'hasMore is true when the first page is a full page (== limit)',
      () async {
        when(
          () => api.getMyCollections(
            type: CollectionType.doing,
          ...
          (_) async => PaginatedCollections(
            items: List.generate(
              20,
              (i) =>
                  MyCollectionSubject(subjectId: i, name: 'A$i', nameCn: 'A$i'),
            ),
            total: 100,
          ),
        );
    ...
    test(
      'hasMore is false when the first page is a short page (< limit)',
      () async {
        when(
          () => api.getMyCollections(
            type: CollectionType.doing,
          ...
          (_) async => const PaginatedCollections(
            items: [
              MyCollectionSubject(subjectId: 1, name: 'A', nameCn: 'A-cn'),
            ],
            total: 1,
          ),
        );
  ...
  group('loadMore', () {
    test(
      'fetches the next page using the current length as offset and appends it',
      () async {
        when(
          () => api.getMyCollections(
        ...
        when(
          ...
          (_) async => const PaginatedCollections(
            items: [
              MyCollectionSubject(subjectId: 20, name: 'B', nameCn: 'B-cn'),
            ],
            total: 21,
          ),
        );
        ...
        await container
            .read(
              myCollectionsControllerProvider(
                type: CollectionType.wish,
              ).notifier,
            )
            .loadMore();
        ...
        final result = container
            .read(myCollectionsControllerProvider(type: CollectionType.wish))
            .value!;
    ...
    test(
      'hasMore becomes false once loadMore returns a short/final page',
      () async {
        when(
          () => api.getMyCollections(
            type: CollectionType.wish,
    ...
    test(
      'hasMore remains true when loadMore returns another full page',
      () async {
        when(
          () => api.getMyCollections(
            type: CollectionType.wish,
  ...
  group('imageUrls', () {
    test(
      'merges cached image URLs from the repository into the page',
      () async {
        when(
          () => api.getMyCollections(
            type: CollectionType.doing,
        ...
        when(
          () => imageCacheRepo.getFor([1]),
        ).thenAnswer((_) async => {1: 'https://example.com/a.jpg'});
    ...
    test(
      'imageUrls has no entry for subjects without a cached image',
      () async {
        when(
          () => api.getMyCollections(
            type: CollectionType.doing,
    ...
    test(
      "loadMore merges the new page's cached image URLs on top of the existing ones",
      () async {
        when(
          () => api.getMyCollections(
            type: CollectionType.wish,
        ...
        when(
          ...
          (_) async => const PaginatedCollections(
            items: [
              MyCollectionSubject(subjectId: 2, name: 'B', nameCn: 'B-cn'),
            ],
            total: 2,
          ),
        );
        when(
          () => imageCacheRepo.getFor([2]),
        ).thenAnswer((_) async => {2: 'https://example.com/b.jpg'});
    ...
    test(
      'build does not fail the whole page load when the image cache read throws',
      () async {
        when(
          () => api.getMyCollections(
            type: CollectionType.doing,
